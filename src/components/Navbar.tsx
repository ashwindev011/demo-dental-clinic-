...
export default function Navbar() {
  ...
  return (
    <>
      <header 
        className={`fixed top-4 left-4 right-4 md:top-0 md:left-0 md:right-0 z-50 transition-all duration-300 px-4 py-3 rounded-2xl md:rounded-none ${
          scrolled 
            ? 'glass-effect border border-primary/10 md:border-b md:border-x-0 md:border-t-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-xl shadow-lg' 
            : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Desktop Nav */}
          <nav className="hidden md:flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-full border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-md">
            {navLinks.map(link => {
